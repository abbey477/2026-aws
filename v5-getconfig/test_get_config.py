...
def _set_aws_env(monkeypatch):
    """
    Set dummy AWS env vars so boto3 doesn't hang trying to resolve
    real credentials from EC2 metadata / config files.

    Used by tests that create MagicMock boto3 clients instead of going
    through moto (which sets these automatically).
    """
...
def _mock_write_log():
    ...
    Yields the mock so tests can optionally assert call args.
    """
...
def patched_handler(dynamodb_tables, monkeypatch):
    ...
    Returns the dynamodb resource so tests can seed additional data.
    """
    monkeypatch.setattr(
...
class TestGetConfigLambda:
    ...
    def test_dynamodb_client_error_propagates(self, monkeypatch, _mock_write_log):
...
class TestGetJobRun:
    ...
    def test_raises_on_client_error(self, monkeypatch):
        ...
        with pytest.raises(ClientError):
            get_job_run(fake_table, "run-001")
...
class TestGetJobConfig:
    ...
    def test_raises_on_client_error(self, monkeypatch):
        ...
        with pytest.raises(ClientError):
            get_job_config(fake_table, "client", "sourcing")
